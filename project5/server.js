...
//  Helpers to load/save users from file
function loadUsers() {
  // Load user data 
  try {
...
function saveUsers(users) {
  // Save users back to database.txt with nice formatting
...
app.post("/signup", (req, res) => {
  // Save new user with birthday info — BaZi needs year/month/day/hour
  const { username, password, year, month, day, hour } = req.body;
  const users = loadUsers();
  if (users.find(u => u.username === username)) {
    return res.render("signup", { error: "用户名已存在" });
  }
  users.push({ username, password, birthday: { year, month, day, hour }, previous: [] });
  saveUsers(users);
...
app.post("/result", ensureLoggedIn, async (req, res) => {
  ...
  // Get today’s alcohol + allergy preferences from form, as i considered each day people's preferences on alcohol or not are not the same
  ...
  if (needsUpdate) {
    ...
    shuffle(drinkList); // Shuffle to avoid always picking the same top match

    for (let d of drinkList) {
      const detail = await fetchDrinkDetail(d.idDrink);
      if (!detail) continue;

      // Skip if it contains an allergy
      const hasAllergy = detail.ingredients.some(ing => allergyList.includes(ing.toLowerCase()));
      if (hasAllergy) continue;
      // Keep only drinks with the element you need today(based on the previous calculation of your bazi and your personal elements that u belongs to.)
      ...
      todayRecord.drink = drink; // Save drink of the day, as if you are in the other page and you wanna back to Today's drink, i can't ask the system to genertate a new one, it's too long and may not be the same one that you generates before.
      saveUsers(users); // Update user history.
      break; //Only need one match each day
    }
  }
